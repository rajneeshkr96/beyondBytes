        ...
        <section className='w-10/12 max-sm:w-full border-b-2 mx-auto'>
         {head?.title && head.actionBtn && <div className='flex justify-between items-center'>
          ...
          <div className={`flex gap-x-6 px-6 mt-6 flex-wrap ${className}`}>
            {menuOptions.map((action,index) => <button key={index} onClick={()=>setActive(index)} className={`capitalize py-8 px-2 transition-colors duration-200 ease-in ${index === active?" text-[#333] border-b-2 max-sm:w-11/12 border-black":"text-gray-500"}`}>{action.action}</button>)}
          </div>
        </section>
